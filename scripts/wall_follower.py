...
# The car runs PD control at 150 Hz, with estimation for positions based on velocity estimates,
# But this is not accurate because of varying velocities and inaccurate scaling between vehicle velocity and velocity signal from code.
###################################################
...
## Communication related
 ...
 # counter for gas rate
        self.gas_cnt = 0
    ## File for stroing data
        self.f_handle = file('/home/nvidia/wsps/rally_ws/src/wall_follower/resources/wallfollower_err.txt','a')
    ...
    def wall_follow(self):
        ...
        while not rospy.is_shutdown():
            ...
            if(abs(steer_wrt) > 1800):
                ...
                if(self.gas_cnt == 4):
                    ...
                    gasvel_wrt = 400
            # Estimation - not very good because speed is varying
            ...
            # compute the yaw from set steer angle (later update to amcl reading)
            self.car_yaw = steer_ang*math.pi/180 #(scale steer to deg then convert to radian)
            # find dx and dy values and find the new distance from wall. 
            ...
            np.savetxt(self.f_handle,np.array([rospy.get_time(),self.steer]),fmt='%5.5f', delimiter=',')
            rt.sleep()
        ...
        if rospy.is_shutdown():
            ...
            try:
                ...
                ser_handle.close()
                print('Closing file and finishing!!')
                self.f_handle.close()
            except:
