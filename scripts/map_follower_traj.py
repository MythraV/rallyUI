...
## Communication related
    ...
    # The gas and steer computed should be scaled to:
        ...
        self.gas_min = -2048
        self.gas_max = 2048  
        self.gas_turn = 350 # Max gas during steep turns
 ...
 # limit for waypoint update
        self.waypt_lim = 5  #2m limit, if dist between des and present is > waypt_lim then do not update
 # Servo trim reading 
        self.trim = 1500    # Default or zero value
...
#-------------------------------------------------------------------------------------
# The trim callback function
    def trim_callback(self,data):
        trim_val = int(data.data)
        if(abs(self.trim - trim_val)>20):
            self.trim = trim_val 
            rospy.loginfo(self.trim)    
        #rospy.loginfo(trim_val)
...
# The main wall_following function
    def map_follow(self):
        ...
        rospy.Subscriber("imu2_dat", String, self.trim_callback)        
        ...
        self.waypt_buf = np.loadtxt('/home/nvidia/wsps/ui_ws/src/wall_follower/resources/ui_speedway.txt')
        ...
        #self.pgas_err = np.sqrt(math.pow((self.car_ax-self.waypt_xd),2)+math.pow((self.car_ay-self.waypt_yd),2))
        #self.pgas_err = 0.1 #start 
        #----------------------- Start control loop     
        while not rospy.is_shutdown() and (run_flag==1):
            ...
            # waypoint updates
            #if abs(self.dis_err-self.pgas_err) > 0:
            #    print((self.dis_err-self.pgas_err))
            if(((rospy.get_time()-self.waypt_ptime)  > (1.0/self.waypt_rate)) and\
             self.trim > 1520): # and self.dis_err < self.waypt_lim and ):
...
###########################
         ...
         # for low velocities needed set 0 every 6 cycles
            if(abs(steer_wrt) > 1900):
                self.gas_cnt = self.gas_cnt+1
                if(self.gas_cnt == 20):
                    self.gas_cnt = 0
                    gasvel_wrt = 0
                else:
                    gasvel_wrt = self.gas_turn
            if(gasvel_wrt > self.gas_max*self.vlim):
                gasvel_wrt = self.gas_max * self.vlim          
